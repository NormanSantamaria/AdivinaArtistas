...
class MainActivity : AppCompatActivity() {
    ...
    fun clickLetra(btnClicked:Button) {
        if (!finalizado) {
            // Obtener el índice de la letra seleccionada inicialmente
            var starIndex = 0
            var resIndex = respuesta.indexOf(btnClicked.text.toString())
            // Si el índice ya fue ocupado entonces no tomar en cuenta los índices hacia atrás
            ...
            if (resIndex != -1) {
                ...
                Toast.makeText(applicationContext, "No es una letra válida", Toast.LENGTH_SHORT).show()
    ...
    fun reiniciarJuego(view: View) {
        ...
        // Elimina las vistas anteriores en flexResponse y flexAlfabeto
        flexResponse.removeAllViews()
        flexAlfabeto.removeAllViews()

        // Restablece la visibilidad de las vistas
        ...
        // Vuelve a generar el alfabeto y las vistas
        val alfabeto = generarAlfabeto(respuesta)
        val alfabetoDesorden = desordenar(alfabeto)
        mostrarEspacioRespuesta(respuesta.length, flexResponse)
        mostrarAlfabeto(alfabetoDesorden.uppercase(), flexAlfabeto)
